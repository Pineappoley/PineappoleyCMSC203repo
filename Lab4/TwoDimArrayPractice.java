...
public class TwoDimArrayPractice extends JFrame
{
   ...
   public void setValues( int value, int row )
   {
    ...
    // Write your code here:
		    ...
		    if(row >= 0 && row < intArray.length) {
		        for (int column = 0; column < intArray[row].length; column++) {
		            intArray[row][column] = value; // Set the value at the current position to the specified value
		            animate(row, column); // Animate the setting of the value
		        }
		        repaint(); // Ask for a repaint after the values have been set
		    } else {
		        // Handle the case where the row index is out of bounds
		        JOptionPane.showMessageDialog(null, "Row index is out of bounds. Valid rows are 0 to " + (intArray.length - 1));
		    }
		}
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
		    ...
		    // Loop through the rest of the rows to find the minimum value in the specified column.
		    for (int row = 1; row < intArray.length; row++) {
		        if (intArray[row][column] < minimum) {
		            minimum = intArray[row][column];  // Found a new minimum
		        }
		        // Animate each comparison by highlighting the current value being checked.
		        animate(row, column, minimum);
		    }
   ...
   */
   public int countFound( int value )
   {
    ...
    // Write your code here:
	   int count = 0; // Initialize the count of the value

	    // Loop through each element of the intArray to count occurrences of value
	    for (int row = 0; row < intArray.length; row++) {
	        for (int column = 0; column < intArray[row].length; column++) {
	            if (intArray[row][column] == value) {
	                count++; // Increment count when value is found
	            }
	            // Animate each check by highlighting the current element being checked
